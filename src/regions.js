...
async function main() {
  const body = await request(
    'https://www.credit-agricole.fr/particulier/acces-cr.html'
  )

  const $ = cheerio.load(body)
  const script = Array.from($('script')).find(script => {
    return $(script).html().includes('regionalBankId')
  })
  ...
  let re =
    /NPC.listCr\[\d+\] = {regionalBankId: \d+, regionalBankName: "(.*)", regionalBankUrlPrefix: "\/(.*)\/" };/g
  let m
  const cr = []
  do {
